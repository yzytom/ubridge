...
SOCKET ip_listen(char *ip_addr, int port, int sock_type, int max_fd, SOCKET fd_array[])
{
	...
	if ((error = getaddrinfo(addr, port_str, &hints, &res0)) != 0) {
		fwprintf(stderr, L"ip_listen: %s", gai_strerror(error));
		return(-1);
	}
	...
	for (res = res0; (res && (nsock < max_fd)); res = res->ai_next)
	{
		...
		setsockopt(fd_array[nsock], SOL_SOCKET, SO_REUSEADDR, &(char)reuse, sizeof(reuse));

		if ((bind(fd_array[nsock], res->ai_addr, res->ai_addrlen) < 0) ||
			((sock_type == SOCK_STREAM) && (listen(fd_array[nsock], 5) < 0)))
		{
			closesocket(fd_array[nsock]);
			fd_array[nsock] = -1;
			continue;
		}
...
int hypervisor_send_reply(hypervisor_conn_t *conn, int code, int done, char *format, ...)
{
	...
	if (conn != NULL) {
		...
		va_start(ap, format);
		char buf[255] = { 0 };
		n += sprintf(buf, "%3d%s", code, (done) ? "-" : " ");
		n += vsprintf(buf + n, format, ap);
		n += sprintf(buf + n, "\r\n");
		send(conn->client_fd, buf, strlen(buf) + 1, 0);
		va_end(ap);
	}
	return(n);
}
...
DWORD WINAPI hypervisor_thread(void *arg)
{
	hypervisor_conn_t  *conn = arg;
	char buffer[512] = { 0 }, **tokens;
	...
	while (conn->active) {
		if (!recv(conn->client_fd, buffer, sizeof(buffer), 0))
			break;
		...
		if (res != 0) {
			...
			global_lock = CreateSemaphore(NULL, 0, 1, NULL);
			hypervisor_exec_cmd(conn, tokens[0], tokens[1], ctx.tok_count - 2, &tokens[2]);
			ReleaseSemaphore(global_lock, 1, NULL);
			CloseHandle(global_lock);
...
static void hypervisor_close_conn(hypervisor_conn_t *conn)
{
	if (conn != NULL) {
		...
		WaitForSingleObject((HANDLE)conn->tid, INFINITE);
...
static hypervisor_conn_t *hypervisor_create_conn(int client_fd)
{
	...
	if (CreateThread(NULL, 0, hypervisor_thread, conn, 0, NULL) == FALSE)
		hypervisor_close_conn(conn);
...
{
	SOCKET fd_array[HYPERVISOR_MAX_FD];
	struct sockaddr_storage remote_addr;
	socklen_t remote_len;
	struct timeval tv;
	int i, res, fd_count, fd_max;
	SOCKET clnt;
	fd_set fds;
