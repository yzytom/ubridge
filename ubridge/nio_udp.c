...
static void nio_udp_free(nio_udp_t *nio_udp)
{
   ...
   if (nio_udp->fd != -1)
	   closesocket(nio_udp->fd);
}

static SSIZE_T nio_udp_send(nio_udp_t *nio_udp, void *pkt, SSIZE_T pkt_len)
{
   return (send(nio_udp->fd, pkt, pkt_len, 0));
}

static SSIZE_T nio_udp_recv(nio_udp_t *nio_udp, void *pkt, SSIZE_T max_len)
{
