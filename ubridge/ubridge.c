...
static void free_bridges(bridge_t* bridge)
{
	...
	while (bridge != NULL) {
		...
		if (bridge->source_tid) {
			WaitForSingleObject(bridge->source_tid, INFINITE);
			CloseHandle(bridge->source_tid);
		}
		if (bridge->destination_tid) {
			WaitForSingleObject(bridge->destination_tid, INFINITE);
			CloseHandle(bridge->destination_tid);
		}
...
static void create_threads(bridge_t *bridge)
{
	while (bridge!= NULL) {
		if (bridge->name != NULL) {
			...
			if (bridge->source_tid == 0)handle_error_en(GetLastError(), "pthread_create");
			bridge->destination_tid = CreateThread(NULL, 0, destination_nio_listener, bridge, 0, NULL);
			if (bridge->destination_tid == 0)handle_error_en(GetLastError(), "pthread_create");
			bridge = bridge->next;
		}
		break;
	}
...
DWORD WINAPI run_hypervisor_thread(run_hypervisor_t p) {
	run_hypervisor(p.hypervisor_ip_address, p.hypervisor_tcp_port);
}
static void ubridge(char *hypervisor_ip_address, int hypervisor_tcp_port)
{
	if (hypervisor_mode) {
		run_hypervisor_t p = { hypervisor_ip_address ,hypervisor_tcp_port};
		HANDLE handle_in = GetStdHandle(STD_INPUT_HANDLE); 
		//SetConsoleMode(handle_in, ENABLE_MOUSE_INPUT | ENABLE_WINDOW_INPUT | ENABLE_PROCESSED_INPUT);
...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-H") == 0) {
			...
			addrport[2] = "\0";
			hypervisor_ip_address = addrport[0];
			if (addrport[1] == NULL) 
				addrport[1] = "12345";
